...
class MysqlFkSiteUser extends Migration
{
    use Utils;
    ...
    public function up()
    {
        Schema::connection('mysql')->table('site_user', function (Blueprint $table) {
            if (!Utils::hasForeignKey('site_user', 'site_user_site_id_foreign', 'mysql')) {
                $table->foreign('site_id')->references('id')->on('site')
                    ->onUpdate('NO ACTION')->onDelete('CASCADE');
            }
            if (!Utils::hasForeignKey('site_user', 'site_user_user_id_foreign', 'mysql')) {
