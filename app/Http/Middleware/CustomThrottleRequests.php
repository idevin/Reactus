...
class CustomThrottleRequests extends ThrottleRequests
{
    ...
    public function handle($request, Closure $next, $maxAttempts = 60, $decayMinutes = 1, $prefix = '')
    {
        ...
        if ($this->tooManyAttempts($key, $maxAttempts, $decayMinutes)) {
            ...
            if (env('APP_DEBUG_VARS') == true) {
                debugvars('MAX ATTEMPTS REACHED: ' . $maxAttempts);
            }

            if ($request->isJson()) {
                $headers = $this->buildJsonHeaders($key, $maxAttempts);
                return $this->error('Слишком много запросов', null, 429, null, $headers);
            } else {
    ...
    protected function buildJsonHeaders($key, $maxAttempts)
    {
