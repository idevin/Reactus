...
<head>
    ...
    <title>
        ...
        @if(isset($title))
            {{$title}}
        @else
            {{strip_tags(\Session::get('site')->title)}}
        @endif
...
{{--    <script src="https://www.google.com/recaptcha/api.js?onload=onloadCallback&render=explicit" async defer></script>--}}
    @if (\Session::get('site') && \Session::get('site')->favicon)
        <link rel="icon" type="image/png" href="{{\Session::get('site')->imageUrl('180x180', 'favicon')}}" sizes="180x180">
    @endif
</head>
